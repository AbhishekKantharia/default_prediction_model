...
run_id = tracker.current_run.info.run_id if tracker.current_run else 'None'
print('  Active run ID: %s' % run_id)
...
rec_rate = max(p.get('recommended_rate', 0.12), 0.06)
emi = emi_calc.compute_emi(500000, rec_rate, 60)
print('  EMI (at %.1f%%): INR %s' % (rec_rate * 100, '{:,.0f}'.format(emi)))
